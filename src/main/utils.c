...
inlinable byte point_exists_at(const vec pos, const double z) {
	const byte point_val = map_point(current_level.wall_data, pos[0], pos[1]);
	return point_val && z < current_level.get_point_height(point_val, pos);
}
