...
void specify_cascade_count_before_any_shader_compilation(const GLsizei num_cascades) {
	FILE* const file = open_file_safely(ASSET_PATH("shaders/shadow/num_cascades.geom"), "w");
...
CascadedShadowContext init_shadow_context(
	...
	return (CascadedShadowContext) {
		...
		.depth_shader = init_shader(ASSET_PATH("shaders/shadow/depth.vert"),
			ASSET_PATH("shaders/shadow/depth.geom"), ASSET_PATH("shaders/shadow/depth.frag")
		),
