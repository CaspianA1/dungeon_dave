...
void set_statemap_area(const StateMap statemap, const buffer_size_t area[4]) {
	...
	/* TODO: make this more efficient by finding a way to set many bits at once across (with an integer
	type that's as wide as possible, or specify the bit width via a typedef or a creation parameter) */
