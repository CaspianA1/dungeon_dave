typedef struct {
	...
	const vec pos;
	const double p_height, pace, horizon_line;
	const int y_pitch;
...
void* floorcast_caller(void* const void_fcp) {
	...
	const int start = fcp.horizon_line - fcp.pace, end = settings.screen_height - fcp.pace;
...
void manage_floorcast(const byte floor_height, const vec pos,
		const double p_height, const double pace, double horizon_line, const int y_pitch) {
	...
	FloorcastCallerParams fcp = {
		...
		.pos = pos, .p_height = p_height, .pace = pace, .horizon_line = horizon_line, .y_pitch = y_pitch
	};
