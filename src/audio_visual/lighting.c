...
void init_lightmap(void) {
	const byte samples_per_tile = 10;
	...
	for (int y = 0; y < lightmap_size.y; y++) {
		pos[1] = (double) y / samples_per_tile;
		for (int x = 0; x < lightmap_size.x; x++) {
			pos[0] = (double) x / samples_per_tile;

			lightmap[y * lightmap_size.x + x] = current_level.shader(pos);
		}
	}
...
inlinable double calculate_shade(const double wall_h, const vec pos) {
	...
	// double shade = current_level.shader(pos) / 10;
