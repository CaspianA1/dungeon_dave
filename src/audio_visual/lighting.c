...
static const byte lightmap_samples_per_tile_axis = 50, /* 15 */ perlin_amplitude = 10;
static const double shader_downscaler = 0.15, perlin_downscaler = 0.3, perlin_frequency = 0.01;

Lightmap init_lightmap(void) {
	Lightmap lightmap = {
		.size = (ivec) {
			current_level.map_size.x * lightmap_samples_per_tile_axis,
			current_level.map_size.y * lightmap_samples_per_tile_axis
		}
	};
	...
	for (int y = 0; y < lightmap.size.y; y++) {
		pos[1] = (double) y / lightmap_samples_per_tile_axis;
		for (int x = 0; x < lightmap.size.x; x++) {
			pos[0] = (double) x / lightmap_samples_per_tile_axis;
...
byte shade_at(const double wall_h, const vec pos) {
	...
	const vec lightmap_pos = (pos - vec_fill(almost_almost_zero)) * vec_fill(lightmap_samples_per_tile_axis);
