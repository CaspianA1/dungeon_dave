...
typedef struct {
	const GLuint scene_shader, depth_map_texture;
	GLuint scene_vbo, depth_map_framebuffer;

	buffer_size_t num_scene_vertices;
	const buffer_size_t shadow_size[2];
} SceneState;

const char *const demo_22_scene_vertex_shader =
	"#version 330 core\n"
	...
	"uniform mat4 scene_model_view_projection;\n"

	"void main() {\n"
		"fragment_color = vertex_pos_world_space / 8.0f;\n"
		"gl_Position = scene_model_view_projection * vec4(vertex_pos_world_space, 1.0f);\n"
	"}\n",

*const demo_22_scene_fragment_shader =
	"#version 330 core\n"
...
GLuint init_demo_23_scene_vbo(buffer_size_t* const num_scene_vertices) {
	...
	*num_scene_vertices = sizeof(vertices) / sizeof(vertices[0]) / num_components_per_vertex;
...
StateGL demo_23_init(void) {
	...
	SceneState scene_state = {
		.scene_shader = init_shader_program(demo_22_scene_vertex_shader, demo_22_scene_fragment_shader),
		.depth_map_texture = preinit_texture(TexPlain, TexNonRepeating),
		.shadow_size = {1024, 1024}
	};

	scene_state.scene_vbo = init_demo_23_scene_vbo(&scene_state.num_scene_vertices);

	glUseProgram(scene_state.scene_shader);
...
void demo_23_drawer(const StateGL* const sgl) {
	...
	static GLint scene_model_view_projection_id;
	static byte first_call = 1;

	if (first_call) {
		INIT_UNIFORM(scene_model_view_projection, scene_state.scene_shader);
		init_camera(&camera, (vec3) {0.0f, 1.0f, 0.0f});
		first_call = 0;
	}

	update_camera(&camera, get_next_event(), NULL);
	UPDATE_UNIFORM(scene_model_view_projection, Matrix4fv, 1, GL_FALSE, &camera.model_view_projection[0][0]);
	...
	glDrawArrays(GL_TRIANGLES, 0, scene_state.num_scene_vertices);
}

void demo_23_deinit(const StateGL* const sgl) {
	const SceneState scene_state = *((SceneState*) sgl -> any_data);

	glDeleteProgram(scene_state.scene_shader);
